...
#[wasm_bindgen]
impl TarsEngine {
    // Internal Helper (WASM dışına kapalı)
    pub(crate) fn get_combined_filter(&self, exclude_field: Option<&str>) -> Option<RoaringBitmap> {
    ...
    pub fn get_state(&self, field: &str, value: &str) -> u8 {
        ...
        if let Some(sel_bitmap) = self.selections.get(field) {
            if !sel_bitmap.is_disjoint(val_bitmap) { return 2; }
        }
        ...
        match other_filters {
            None => 1, 
            Some(global_filter) => {
                if !global_filter.is_disjoint(val_bitmap) { 1 } else { 0 }
            }
        }
    }
    ...
    pub fn query_count(&self, field: &str, value: &str) -> u32 {
        ...
        let all_filters = self.get_combined_filter(None);
        match all_filters {
